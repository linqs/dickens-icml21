...
readonly DATA_URL='https://linqs-data.soe.ucsc.edu/public/dickens-icml21/movielens-1m/ml-1m.tar.gz'
readonly DATA_FILE='ml-1m.tar.gz'
readonly EXTRACTED_DATA_FILE='ml-1m'
...
function extract_zip() {
   ...
   echo "Extracting data from ${path} and writing to $( dirname "${expectedDir}" )"
   tar -xvf "${path}" -C "$( dirname "${expectedDir}" )"
   if [[ "$?" -ne 0 ]]; then
