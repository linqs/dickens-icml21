...
readonly ADDITIONAL_SERVER_OPTIONS=''
...
function runServer() {
   ...
   java -Xmx24G -Xms24G -jar "${JAR_PATH}" --model "${BASE_NAME}.psl" --data "${BASE_NAME}-eval.data" --output inferred-predicates ${ADDITIONAL_EVAL_OPTIONS} ${ADDITIONAL_SERVER_OPTIONS} ${ADDITIONAL_PSL_OPTIONS} "$@"
   if [[ "$?" -ne 0 ]]; then
