...
readonly DATA_CONSTRUCTION_SCRIPT="./construct.sh"
...
function main() {
   ...
   construct_commands "${SCRIPTS_DIR}" "${DATA_CONSTRUCTION_SCRIPT}"
}

function construct_commands() {
   local scripts_dir=$1
   local data_construction_script=$2

   pushd . > /dev/null
       cd "${scripts_dir}"
       ${data_construction_script}
   popd > /dev/null
}
